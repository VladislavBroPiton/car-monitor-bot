...
// ── Группировка по дням ──────────────────────────────────────────────────────
// Берём полдень по Москве, а не «через N часов»: под вечер сдвиг на пару
// часов уезжает на следующие сутки, и проверка ложно падала
const mskNoon = shiftDays => {
  const msk = new Date(Date.now() + shiftDays * 86400000)
      .toLocaleDateString('en-CA', {timeZone: 'Europe/Moscow'})
  return `${msk} 09:00:00+00:00`      // 12:00 по Москве
}
eq(cpDayLabel(mskNoon(0)), 'Сегодня', 'сегодняшние торги')
eq(cpDayLabel(mskNoon(1)), 'Завтра', 'завтрашние торги')
