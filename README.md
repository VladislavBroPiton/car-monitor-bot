...
Telegram-бот для мониторинга лотов аукциона Copart — битых и залоговых авто
из США и Канады.

Российские площадки (Auto.ru, Авито, Дром) убраны: их парсеры удалены,
а прежние фильтры и объявления вычищаются из базы при первом запуске
(`db/repository.py` → `_DROP_RU`).
...
- **aiohttp** — запросы к публичному JSON API Copart
...
SCRAPER_API_KEY=       # ScraperAPI — запасной путь к Copart (опционально)
USD_RUB_RATE=90        # запасной курс, если ЦБ недоступен (по умолчанию 90)
```
...
- **Цена** — `la`, оценочная стоимость в **долларах** (у канадских лотов — CAD).
  ...
  Границы цены в фильтре задаются в тех же долларах — без пересчёта.
- **Пробег** — `orr`, в **милях**. Границы пробега в фильтре тоже в милях.
...
Создаётся кнопкой «➕🟡 Новый фильтр Copart» в главном меню — мастер из 12 шагов.
Значения задаются в единицах самого аукциона, пересчёта нигде нет:

| Поле | Единица |
|------|---------|
| Цена от/до | **доллары** |
| Пробег до | **мили** |
| Марка, модель | латиница, из справочника аукциона |
| Площадки | штаты и провинции (`FL`, `TX`, `ON`…) |

Марку и модель можно отметить сразу несколько — они складываются по ИЛИ.
Правка марки у готового фильтра обновляет и одиночное поле `brand`,
и список `brands`: парсер читает список, и без этого правка ничего бы
не изменила.
...
python tests/test_auth.py      # подпись initData
...
## Уборка после российских площадок

При первом старте новой версии бот удаляет фильтры с `kind <> 'copart'`
и все записи источников `autoru`, `avito`, `drom` — из `seen_listings`,
`user_seen`, `favorites`, `price_history` и `source_health`. Запросы
идемпотентны: со второго раза они ничего не находят. Те же запросы лежат
в `db/schema.sql`, если уборку удобнее сделать руками в SQL-редакторе Neon.

## Несколько пользователей

Бот работает с любым числом пользователей: у каждого свои фильтры, избранное,
уведомления и настройки тихих часов.

**Как устроено.** `seen_listings` — общий каталог лотов: один лот лежит там
в одном экземпляре, сколько бы человек его ни нашли. Кому какой лот
отправляли, хранится отдельно в `user_seen`. Так лот, найденный одним
пользователем, не пропадает у остальных — раньше признак «виденное» жил
прямо в `seen_listings` и делал многопользовательский режим невозможным.

**Доступ.** Пользователь заводится автоматически при первом `/start`.
Владельцу (`OWNER_ID`) приходит уведомление о новом человеке.
Отключить или вернуть доступ — команда `/users` у администратора.

**Mini App.** Раньше API отвечал данными владельца всем подряд. Теперь каждый
запрос проверяется по подписанной строке `initData`, которую Telegram выдаёт
странице: подпись считается ключом от токена бота, подделать её нельзя.
Без валидной подписи API возвращает 401. Реализация — `auth.py`,
проверка покрыта тестами в `tests/test_auth.py`.

При первом запуске после обновления вся прежняя история закрепляется
за владельцем, поэтому заново она ему не приходит.

## Курс валют и здоровье источника
...
Курс нужен в двух местах: расчёт «под ключ» и порог цены в настройках
(он задаётся в рублях, а лоты в долларах).

`source_health` считает, сколько обходов подряд аукцион вернул пусто.
После трёх подряд бот пишет владельцу — иначе смена формата ответа
...
|---------|----------|
| /start | Приветствие |
| /filters | Управление фильтрами аукциона |
