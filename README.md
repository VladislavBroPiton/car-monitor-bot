...
Офлайн, сеть не нужна: разбор проверяется на зафиксированной выдаче API
в `tests/fixtures/copart_search.json`. Ловят молчаливую поломку парсера —
когда бот продолжает работать, но шлёт карточки без цены, фото и повреждений.
